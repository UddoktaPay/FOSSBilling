...
class Payment_Adapter_UddoktaPay extends Payment_AdapterAbstract implements \FOSSBilling\InjectionAwareInterface
{
    ...
    public function getHtml($api_admin, $invoice_id, $subscription)
    {
        ...
        $data = $this->upGetPaymentFields($invoice);

        $url = $this->upInitPayment($data);

        return $this->upGenerateForm($url);
    }
     ...
     * @var array $data
     */
    private function upVerifyPayment($data)
    {
    ...
    public function upIsIpnDuplicate($response)
    {
        $sql = 'SELECT id
                FROM transaction
    ...
    public function upCurrencyConvert($amount, $currency = null)
    {
        if ($currency == 'BDT') {
            return $amount;
        }
    ...
    public function upCurrencyConvertForTransaction($amount, $currency = null)
    {
        if ($currency == 'BDT') {
            return $amount;
        }
